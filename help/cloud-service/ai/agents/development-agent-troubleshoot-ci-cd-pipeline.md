...
>[!TIP]
>
> Siehe auch [Überblick über Agenten in AEM](https://experienceleague.adobe.com/en/docs/experience-manager-cloud-service/content/ai-in-aem/agents/overview), um eine vollständige Liste der in AEM as a Cloud Service verfügbaren Agenten, ihrer Funktionen und ihrer Zugriffsmöglichkeiten zu erhalten.
...
- Erstellen Sie ein neues Cloud Manager-Git-Repository für das WKND Sites-Projekt und fügen Sie es als Remote-Datenquelle zum lokalen Git-Repository hinzu:
   ...
   - Geben Sie einen **Repository-Namen** ein (z. B. „wknd-site-tutorial„) und klicken Sie auf **Speichern**. Warten Sie, bis das Repository erstellt wurde.
     ...
     ![Repository hinzufügen](./assets/development-agent-troubleshoot-ci-cd-pipeline/add-repository.png)

   - Klicken Sie **oben rechts auf** Repository-Informationen abrufen“ und kopieren Sie die Repository-URL.

     ![Auf Repository-Informationen zugreifen](./assets/development-agent-troubleshoot-ci-cd-pipeline/access-repo-info.png)
...
- Konfigurieren **im Dialogfeld „Produktionsfremde Pipeline**&quot; Folgendes:
   ...
   - **Konfiguration** Schritt:
     ...
     ![Hinzufügen einer produktionsfremden Pipeline-Konfiguration](./assets/development-agent-troubleshoot-ci-cd-pipeline/add-non-production-pipeline-configuration.png)
   ...
   - **Source-Code** Schritt:
     ...
     ![Hinzufügen des Source-Codes für produktionsfremde Pipelines](./assets/development-agent-troubleshoot-ci-cd-pipeline/add-non-production-pipeline-source-code.png)
...
- Führen Sie die neu erstellte Code-Qualitäts **Pipeline aus, indem Sie** Menü mit den drei Punkten des Pipeline-Eintrags auf „Ausführen“ klicken.
  ...
  ![Ausführen der Code-Qualitäts-Pipeline](./assets/development-agent-troubleshoot-ci-cd-pipeline/run-code-quality-pipeline.png)
...
![Fehlgeschlagene Pipeline-Ausführung](./assets/development-agent-troubleshoot-ci-cd-pipeline/failed-pipeline-execution.png)
...
- Geben Sie die Details zum Pipeline-Fehler in natürlicher Sprache (**)**. Zum Beispiel:
  ...
  ![AEM-Entwicklungsagenten aufrufen](./assets/development-agent-troubleshoot-ci-cd-pipeline/invoke-aem-development-agent.png)
...
- Sobald die Argumentation abgeschlossen ist, klicken Sie auf das Symbol **Im Vollbildmodus öffnen**, um den detaillierten Fehlerbehebungsprozess anzuzeigen.
  ...
  ![Im Vollbildmodus öffnen](./assets/development-agent-troubleshoot-ci-cd-pipeline/open-in-full-screen.png)
...
- In diesem Fall schlug der Agent korrekt vor, entweder die Implementierung zu ändern (`getName()`) oder den Modultest zu aktualisieren (`getNameTest()`), um das Problem zu beheben. Es vermied Halluzinationen und verwendete einen Human-in-the-Loop-Ansatz, während es verwertbare Code-Änderungen für den Entwickler bereitstellte.
  ...
  ![Code-Änderungen kopieren](./assets/development-agent-troubleshoot-ci-cd-pipeline/copy-code-changes.png)
...
![Verzweigungen vergleichen](./assets/development-agent-troubleshoot-ci-cd-pipeline/compare-branches.png)

Siehe auch [Beispielaufforderungen](https://experienceleague.adobe.com/en/docs/experience-manager-cloud-service/content/ai-in-aem/agents/development/overview#sample-prompts), um weitere Ideen zur Verwendung des AEM-Entwicklungsagenten zu erhalten.
...
- [Übersicht über den Entwicklungsagenten](https://experienceleague.adobe.com/en/docs/experience-manager-cloud-service/content/ai-in-aem/agents/development/overview)
- [Überblick über Agenten in AEM](https://experienceleague.adobe.com/en/docs/experience-manager-cloud-service/content/ai-in-aem/agents/overview)
