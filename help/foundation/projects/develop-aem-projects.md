...
Dies ist ein Entwicklungs-Tutorial, in dem gezeigt wird, wie zu entwickeln für [!DNL AEM Projects].  In diesem Lernprogramm erstellen wir eine benutzerdefinierte Projektvorlage, mit der neue Projekte in AEM zur Verwaltung von Workflows und Aufgaben für das Authoring von Inhalten erstellt werden können.
...
[[!DNL AEM Projects]](https://helpx.adobe.com/experience-manager/6-5/sites/authoring/using/projects.html) ist eine Funktion von AEM, die die Verwaltung und Gruppierung aller Workflows und Aufgaben im Zusammenhang mit der Inhaltserstellung im Rahmen einer AEM Sites- oder Asset-Implementierung erleichtert.
...
In diesem Lernprogramm wird der für die Erstellung einer benutzerdefinierten Projektvorlage erforderliche Code schrittweise erläutert. Sie können das [angehängte Paket](./assets/develop-aem-projects/projects-tasks-guide.ui.apps-0.0.1-SNAPSHOT.zip) auf eine lokale Umgebung herunterladen und installieren, um das Lernprogramm zu befolgen. Sie können auch auf das gesamte Maven-Projekt zugreifen, das auf [GitHub](https://github.com/Adobe-Marketing-Cloud/aem-guides/tree/feature/projects-tasks-guide)gehostet wird.
...
Dieses Tutorial geht von einigen Grundkenntnissen über [AEM Entwicklungspraktiken](https://helpx.adobe.com/experience-manager/6-5/sites/developing/using/the-basics.html) und einer gewissen Vertrautheit mit der [AEM Maven-Projekteinrichtung](https://helpx.adobe.com/de/experience-manager/6-5/sites/developing/using/ht-projects-maven.html)aus. Der gesamte erwähnte Code soll als Referenz verwendet werden und sollte nur in einer [lokalen AEM bereitgestellt werden](https://helpx.adobe.com/experience-manager/6-5/sites/deploying/using/deploy.html#GettingStarted).

## Struktur einer Projektvorlage

Projektvorlagen sollten unter Quellcodeverwaltung gestellt werden und unter Ihrem Anwendungsordner unter /apps leben. Idealerweise sollten sie in einen Unterordner mit der Benennungsregel ***/projects/templates/**&lt;my-template> eingefügt werden. Indem Sie diese Benennungsregel befolgen, stehen Autoren beim Erstellen eines Projekts automatisch alle neuen benutzerdefinierten Vorlagen zur Verfügung. Die Konfiguration der verfügbaren Projektvorlagen ist festgelegt auf: **/content/projects/jcr:content** -Knoten durch die **Eigenschaft cq:allowedTemplates** . Standardmäßig ist dies ein regulärer Ausdruck: **/(apps|libs)/.*/projects/templates/.***

Der Stammknoten einer Projektvorlage hat einen **jcr:primaryType** von **cq:Template**. Unter dem Stammknoten befinden sich 3 Knoten: **Gadgets**, **Rollen** und **Workflows**. Diese Knoten sind alle **nt:unstructured**. Unter dem Stammknoten kann auch eine Datei &quot;thumbnail.png&quot;sein, die angezeigt wird, wenn die Vorlage im Assistenten &quot;Projekt erstellen&quot;ausgewählt wird.
...
Es gibt keine weiteren Eigenschaften auf diesem Knoten, aber die untergeordneten Elemente des Knotens &quot;gadgets&quot;steuern, welche Projektkacheln das Dashboard des Projekts füllen, wenn ein neues Projekt erstellt wird. [Die Projektkacheln](https://helpx.adobe.com/experience-manager/6-5/sites/authoring/using/projects.html#ProjectTiles) (auch Gadgets oder Pods genannt) sind einfache Karten, die den Arbeitsplatz eines Projekts füllen. Eine vollständige Liste der Ottomotoren finden Sie unter: **/libs/cq/gui/components/projects/admin/pod. **Projekteigentümer können Kacheln immer hinzufügen/entfernen, nachdem ein Projekt erstellt wurde.

### Rollen {#roles}

Es gibt 3 [Standardrollen](https://helpx.adobe.com/experience-manager/6-5/sites/authoring/using/projects.html#UserRolesinaProject) für jedes Projekt: **Beobachter**, **Redakteure** und **Eigentümer**. Durch Hinzufügen untergeordneter Knoten unter dem Knoten &quot;Rollen&quot;können Sie zusätzliche geschäftsspezifische Rollen für die Vorlage hinzufügen. Sie können diese Rollen dann mit bestimmten Workflows verknüpfen, die mit dem Projekt verbunden sind.

### Workflows {#workflows}

Einer der verlockendsten Gründe für die Erstellung einer benutzerdefinierten Projektvorlage ist die Möglichkeit, die verfügbare Workflows für das Projekt zu konfigurieren. Diese können OOTB-Workflows oder benutzerdefinierte Workflows. Unter dem **Workflows** Knoten muss ein **modellbasierter** Knoten (auch `nt:unstructured`) und untergeordnete Knoten vorhanden sein, unter denen die verfügbaren Workflow-Modelle angegeben werden. Die Eigenschaft **modelId **verweist auf das Workflow-Modell unter /etc/workflow und der **Eigenschaftenassistent** verweist auf das beim Starten des Workflows verwendete Dialogfeld. Ein großer Vorteil von Projekten besteht darin, dass ein benutzerdefinierter Dialog (Assistent) hinzugefügt werden kann, um geschäftsspezifische Metadaten am Beginn des Workflows zu erfassen, wodurch weitere Aktionen im Workflow ausgeführt werden können.
...
Da wir hauptsächlich Knoten kopieren/konfigurieren werden, verwenden wir CRXDE Lite. Öffnen Sie in Ihrer lokalen AEM die [CRXDE Lite](http://localhost:4502/crx/de/index.jsp).

1. Beginn durch Erstellen eines neuen Ordners unter dem `/apps/&lt;your-app-folder&gt;` Namen `projects`. Erstellen Sie einen weiteren Ordner unter dem Namen `templates`.
...
1. Zur Vereinfachung werden wir unsere benutzerdefinierte Vorlage aus der vorhandenen Simple Project-Vorlage Beginn geben.
   ...
   1. Kopieren Sie den Knoten **/libs/cq/core/content/projects/templates/default** unter dem in Schritt 1 erstellten Ordner &quot; *templates* &quot;.
...
1. Sie sollten jetzt einen Pfad wie **/apps/aem-guides/projects-Aufgaben/projects/templates/authoring-project** haben.
   ...
   1. Bearbeiten Sie die Eigenschaften **jcr:title** und **jcr:description** des Knotens author-project in benutzerdefinierte Titel- und Beschreibungswerte.
      ...
      1. Lassen Sie die **Wizard** -Eigenschaft auf die standardmäßigen Projekteigenschaften verweisen.
...
1. Für diese Projektvorlage möchten wir Aufgaben verwenden.
   1. hinzufügen Sie einen neuen Knoten **nt:unstructured** unterhalb von authoring-project/gadgets namens **Aufgaben**.
   1. hinzufügen Sie die Zeichenfolgeneigenschaften zum Knoten Aufgaben für **cardWeight** = &quot;100&quot;, **jcr:title**=&quot;Aufgaben&quot; und **sling:resourceType**=&quot;cq/gui/components/projects/admin/pod/taskpod&quot;.

   Jetzt wird die Kachel [](https://docs.adobe.com/docs/en/aem/6-3/author/projects.html#Tasks) Aufgaben standardmäßig angezeigt, wenn ein neues Projekt erstellt wird.
...
1. Wir fügen unserer Projektvorlage eine benutzerdefinierte Rolle als genehmigende Person hinzu.
   ...
   1. hinzufügen einem anderen **nt:unstructured** -Knoten, der Genehmiger als untergeordnetes Element des Rollenknotens beschriftet.
   1. hinzufügen String-Eigenschaften **jcr:title** = &quot;**Genehmiger**&quot;, **roleclass** =&quot;**owner**&quot;, **roleid******=&quot;Genehmiger&quot;.
      ...
      1. **roleclass** regelt die Berechtigungen, die für diese Rolle auf der Grundlage der [3 OOTB-Rollen](https://docs.adobe.com/docs/en/aem/6-3/author/projects.html#User Rollen in einem Projekt) beantragt werden: **owner**, **editor** und **Beobachter**.
      1. Im Allgemeinen kann die Rollout **Eigentümer sein, wenn die benutzerdefinierte Rolle mehr eine Führungsrolle darstellt.** Wenn es sich um eine spezifischere Authoring-Rolle wie Fotograf oder Designer handelt, sollte eine **Editor** -Rollout ausreichen. Der große Unterschied zwischen **Eigentümer** und **Editor** besteht darin, dass Projektbesitzer die Projekteigenschaften aktualisieren und dem Projekt neue Benutzer hinzufügen können.
...
1. Damit Autoren die Projektvorlage leichter identifizieren können, können Sie eine benutzerdefinierte Miniaturansicht hinzufügen. Die empfohlene Größe beträgt 319 x 319 Pixel.
   1. Erstellen Sie in CRXDE Lite eine neue Datei als Geschwisterdatei mit den Knoten &quot; **thumbnail.png**&quot;, &quot;Rollen&quot;und &quot;Workflows&quot;.
   1. Speichern und navigieren Sie dann zum `jcr:content` Knoten und zur Dublette klicken Sie auf die `jcr:data` Eigenschaft (vermeiden Sie das Klicken auf &#39;Ansicht&#39;).
      1. Dies sollte Sie mit einem Dialogfeld zur Bearbeitung `jcr:data` der Datei auffordern und Sie können eine benutzerdefinierte Miniaturansicht hochladen.
...
* **Threaded Comments** - Autoren arbeiten an einer Aufgabe und können Kommentare hinterlassen, die die Zusammenarbeit verbessern
* **Sichtbarkeit** - Kacheln und Ansichten mit Aufgaben ermöglichen es Managern, Ansichten über die Zeitverwendung zu erhalten
* **Projektintegration** - Aufgaben sind bereits mit Projektrollen und Dashboards integriert
...
**Rush** - Eile Routing erstellt auch eine Aufgabe, die der Projektgenehmigungsgruppe zugewiesen ist. Die Priorität der Aufgabe ist hoch und das Fälligkeitsdatum liegt bei nur 1 Tag.

**Umgehen** - In diesem Beispielarbeitsablauf hat der ursprüngliche Teilnehmer die Möglichkeit, die Validierungsgruppe zu umgehen. (Ja, das könnte den Zweck eines &quot;Genehmigungsarbeitsablaufs&quot;beeinträchtigen, ermöglicht uns jedoch, zusätzliche Routing-Funktionen zu illustrieren)
...
1. Als Best Practice sollten benutzerspezifische Workflows in ihrem eigenen Ordner unter /etc/workflow/models gruppiert werden. Erstellen Sie in CRXDE Lite einen neuen **&#39;nt:folder&#39;** unter /etc/workflow/models mit dem Namen **&quot;aem-guides&quot;**. Durch Hinzufügen eines Unterordners wird sichergestellt, dass benutzerdefinierte Workflows bei Aktualisierungen oder Service Pack-Installationen nicht versehentlich überschrieben werden.
   ...
   *Beachten Sie, dass der Ordner oder die benutzerdefinierte Workflows niemals unter OOOB-Unterordnern wie /etc/workflow/models/dam oder /etc/workflow/models/projects platziert werden müssen, da der gesamte Unterordner auch durch Upgrades oder Service Packs überschrieben werden kann.
   ...
   >[!NOTE]
   >
   >Bei Verwendung von AEM 6.4+ hat sich der Speicherort des Arbeitsablaufs geändert. See [here for more details.](https://helpx.adobe.com/experience-manager/6-5/sites/developing/using/workflows-best-practices.html#LocationsWorkflowModels)

   Bei Verwendung von AEM 6.4+ wird das Workflow-Modell unter `/conf/global/settings/workflow/models`erstellt. Wiederholen Sie die oben genannten Schritte mit dem Ordner &quot;/conf&quot;, fügen Sie einen Unterordner mit dem Namen `aem-guides` hinzu und verschieben Sie den `content-approval-workflow` darunter liegenden Ordner.

   ![Moderner Speicherort](./assets/develop-aem-projects/modern-workflow-definition-location.png)des Workflow-Modells in 6.4+
...
1. In AEM 6.3 wurde die Möglichkeit eingeführt, einem bestimmten Workflow Workflow-Phasen hinzuzufügen. Die Schritte werden dem Benutzer über den Posteingang auf der Registerkarte &quot;Workflow-Info&quot;angezeigt. Es zeigt dem Benutzer die aktuelle Phase des Workflows sowie die Phasen vor und nach der Anwendung an.
   ...
   Optional können Sie ein **Bild** in die Seiteneigenschaften hochladen, das als Workflow-Miniaturansicht verwendet wird, wenn der Benutzer es auswählt. Die Bildabmessungen sollten 319 x 319 Pixel betragen. Das Hinzufügen einer **Beschreibung** zu den Seiteneigenschaften wird auch angezeigt, wenn ein Benutzer den Workflow auswählen möchte.
...
1. Der Arbeitsablaufprozess &quot;Aufgabe erstellen&quot;dient zum Erstellen einer Aufgabe als Arbeitsablaufschritt. Der Workflow wird erst nach Abschluss der Aufgabe weitergeführt. Ein wichtiger Aspekt der Aufgabe &quot;Projekt erstellen&quot;ist, dass sie Workflow-Metadatenwerte lesen und diese zur dynamischen Erstellung der Aufgabe verwenden kann.
   ...
   Löschen Sie zunächst den &quot;Participant Step&quot;, der standardmäßig erstellt wird. Über den Sidekick im Komponentenmenü erweitern Sie die Unterüberschrift **&quot;Projekte&quot;** und ziehen Sie die Aufgabe **&quot;Projekt erstellen&quot;** per Drag &amp; Drop auf das Modell.
   ...
   Diese Registerkarte ist für alle Workflow-Prozessschritte üblich und wir legen den Titel und die Beschreibung fest (diese sind für den Endbenutzer nicht sichtbar). Die wichtige Eigenschaft, die wir einstellen werden, ist die Workflow-Phase, um **&quot;Inhalt bearbeiten&quot;** aus dem Dropdown-Menü.
   ...
   Der Arbeitsablauf zum Erstellen der Aufgabe eines Projekts wurde entwickelt, um eine Aufgabe als Arbeitsablaufschritt zu erstellen. Auf der Registerkarte &quot;Aufgabe&quot;können Sie alle Werte der Aufgabe festlegen. In unserem Fall möchten wir, dass der Bevollmächtigte dynamisch ist, damit wir ihn leer lassen. Die übrigen Eigenschaftswerte:
...
1. Im vorherigen Schritt wurde auf ein Skript zur Aufgabe vor dem Erstellen verwiesen. Wir werden dieses Skript jetzt erstellen, in dem wir den Verantwortlichen der Aufgabe basierend auf dem Wert eines Workflow-Metadatenwerts &quot;**Verantwortlicher**&quot;festlegen. Der Wert **&quot;Verantwortlicher&quot;** wird beim Starten des Workflows festgelegt. Darüber hinaus werden die Workflow-Metadaten gelesen, um die Priorität der Aufgabe dynamisch auszuwählen, indem der &quot;**taskPriority&quot;** -Wert der Workflow-Metadaten sowie der **&quot;taskDueDate&quot; **dynamisch eingestellt werden, wenn die erste Aufgabe fällig wird.
...
1. Navigieren Sie zurück zum Arbeitsablauf für die Inhaltsgenehmigung. Ziehen Sie die **Komponente &quot;OR-Teilung** &quot;(im Sidekick unter der Kategorie &quot;Workflow&quot;) unter den Schritt der **Beginn-Aufgabe** . Wählen Sie im Dialogfeld Allgemein das Optionsfeld für 3 Zweige. Die ODER-Teilung liest den Workflow-Metadatenwert **&quot;lastTaskAction&quot;** , um die Route des Workflows zu bestimmen. Die **&quot;lastTaskAction&quot;** -Eigenschaft wird auf einen der in Schritt 4 konfigurierten Werte aus der Registerkarte &quot;Routing&quot;gesetzt. Für jede der Registerkarten &quot;Verzweigung&quot;füllen Sie den **Skript** -Textbereich mit den folgenden Werten aus:
...
1. Ziehen Sie per Drag &amp; Drop einen weiteren Schritt &quot;Aufgabe **zum** Erstellen eines Projekts&quot;nach links (Zweig 1) unter der ODER-Teilung. Füllen Sie das Dialogfeld mit den folgenden Eigenschaften aus:
...
Kopieren Sie den folgenden Code in den Textbereich &quot; **Skript** &quot;der Registerkarte **Erweiterte Einstellungen **s. Dieser Code liest die Workflow-Metadaten und weist die Aufgabe der Projektgenehmigungsgruppe zu. Wenn der Gruppenwert Genehmiger nicht gefunden wird, wird die Aufgabe der Gruppe Administratoren zugewiesen.
...
1. Ziehen Sie per Drag &amp; Drop einen weiteren Schritt &quot;Projekt-Aufgabe **** erstellen&quot;auf das Modell, um die mittlere Verzweigung (Zweig 2) unter der ODER-Teilung anzuzeigen. Füllen Sie das Dialogfeld mit den folgenden Eigenschaften aus:
   ...
   Wir können dasselbe Skriptfragment wie in Schritt 7 erneut verwenden, um den Textbereich **Skript** auf der Registerkarte** Erweiterte Einstellungen **auszufüllen. Kopieren+Einfügen des folgenden Codes:
...
1. Da die Gruppe &quot;Genehmigende Personen&quot;die Möglichkeit hat, den Workflow für weitere Änderungen an den ursprünglichen Editor zurückzusenden, verlassen wir uns auf den **Goto** -Schritt, um die letzte durchgeführte Aktion zu lesen und den Workflow an den Anfang zu leiten oder ihn fortzusetzen.
   ...
   Das letzte Element, das wir konfigurieren werden, ist das Skript als Teil des Goto-Prozessschritts. Der Skriptwert kann über das Dialogfeld eingebettet oder so konfiguriert werden, dass er auf eine externe Datei verweist. Das Goto-Skript muss eine **Funktion check()** enthalten und true zurückgeben, wenn der Workflow zum angegebenen Schritt gehen soll. Eine Rückgabe von &quot;false&quot;führt dazu, dass der Workflow vorwärts läuft.

   Wenn die genehmigende Gruppe die Aktion **&quot;Zur Überarbeitung zurücksenden&quot;** (konfiguriert in Schritt 7 und 8) wählt, dann möchten wir den Workflow zum **&quot;Erstellen der Beginn-Aufgabe&quot;** -Schritt zurückgeben.
...
1. Zur Veröffentlichung der Nutzlast verwenden wir das OOOB &quot;Seite **aktivieren/Asset** -Prozess&quot;. Dieser Prozessschritt erfordert wenig Konfiguration und fügt die Nutzlast des Workflows zur Replikationswarteschlange zur Aktivierung hinzu. Wir fügen den Schritt unter dem Goto-Schritt hinzu und auf diese Weise kann er nur erreicht werden, wenn die genehmigende Gruppe den Inhalt zur Veröffentlichung genehmigt hat oder der ursprüngliche Editor den Bypass-Genehmigungsweg gewählt hat.

   Ziehen Sie den Schritt **Seite/Asset** -Prozess aktivieren (im Sidekick unter dem WCM-Workflow) unter dem Goto-Schritt im Modell.
...
## Assistenten zum Erstellen des Arbeitsablaufs für Beginn {#start-workflow-wizard}
...
1. In CRXDE-Lite erstellen wir einen Unterordner unter dem `/apps/aem-guides/projects-tasks/projects` Ordner &quot;wizards&quot;. Kopieren Sie den Standardassistenten aus: `/libs/cq/core/content/projects/workflowwizards/default_workflow` unter dem neu erstellten Assistenten-Ordner und benennen Sie ihn in **content-approval-Beginn** um. Der vollständige Pfad sollte nun sein: `/apps/aem-guides/projects-tasks/projects/wizards/content-approval-start`.

   Der Standardassistent ist ein zweispaltiger Assistent mit der ersten Spalte, in der Titel, Beschreibung und Miniaturansicht des Workflow-Modells ausgewählt sind. Die zweite Spalte enthält die Felder für den Workflow-Titel, den Beginn-Kommentar und den Nutzlastpfad. Der Assistent ist ein standardmäßiges Touch-UI-Formular und verwendet zum Ausfüllen der Felder die Standardkomponenten des [Granite-UI-Formulars](https://docs.adobe.com/docs/en/aem/6-5/develop/ref/granite-ui/api/jcr_root/libs/granite/ui/components/coral/foundation/form/index.html) .

   ![Arbeitsablauf für Inhaltsgenehmigung](./assets/develop-aem-projects/content-approval-start-wizard.png)

1. Dem Assistenten wird ein zusätzliches Feld hinzugefügt, mit dem der Verantwortliche der ersten Aufgabe im Workflow festgelegt wird (siehe [Erstellen des Workflow-Modells](#create-workflow-model): Schritt 5).

   Erstellen Sie anschließend `../content-approval-start/jcr:content/items/column2/items` einen neuen Knoten des Typs `nt:unstructured` &quot; **assign&quot;**. Wir werden die Komponente &quot;Projekte-User Picker&quot;verwenden (die auf der Komponente [Granite User Picker basiert](https://docs.adobe.com/docs/en/aem/6-5/develop/ref/granite-ui/api/jcr_root/libs/granite/ui/components/coral/foundation/form/userpicker/index.html)). Mit diesem Formularfeld können Sie die Benutzer- und Gruppenauswahl auf diejenigen beschränken, die zum aktuellen Projekt gehören.
...
1. Wir fügen außerdem ein Feld zur Prioritätenauswahl hinzu, das die Priorität der ersten Aufgabe im Workflow bestimmt (siehe Workflow-Modell [erstellen](#create-workflow-model): Schritt 5).

   Erstellen Sie anschließend `/content-approval-start/jcr:content/items/column2/items` einen neuen Knoten des Typs `nt:unstructured` &quot; **Priorität**&quot;. Das Formularfeld wird mit der [Granite UI Select-Komponente](https://docs.adobe.com/docs/en/aem/6-2/develop/ref/granite-ui/api/jcr_root/libs/granite/ui/components/coral/foundation/form/select/index.html) gefüllt.

   Unter dem **Priority** -Knoten fügen wir einen **items** -Knoten von **nt:unstructured** hinzu. Fügen Sie unter dem Knoten **items** 3 weitere Knoten hinzu, um die Auswahloptionen für Hoch, Mittel und Niedrig auszufüllen. Jeder Knoten ist vom Typ **nt:unstructured** und sollte eine **text** - und **value** -Eigenschaft haben. Sowohl Text als auch Wert sollten mit dem gleichen Wert übereinstimmen:

   1. Hoch
   1. Mittel
   1. Niedrig

   Fügen Sie für die Node Medium eine zusätzliche boolesche Eigenschaft mit dem Namen &quot;**selected&quot;** mit dem Wert **true** hinzu. Dadurch wird sichergestellt, dass Medium der Standardwert im Auswahlfeld ist.
...
1. Der Workflow-Initiator kann das Fälligkeitsdatum der ersten Aufgabe festlegen. Diese Eingabe wird im Formularfeld [Granite UI DatePicker](https://docs.adobe.com/docs/en/aem/6-5/develop/ref/granite-ui/api/jcr_root/libs/granite/ui/components/coral/foundation/form/datepicker/index.html) erfasst. Außerdem wird ein unsichtbares Feld mit einem [TypeHint](https://sling.apache.org/documentation/bundles/manipulating-content-the-slingpostservlet-servlets-post.html#typehint) hinzugefügt, um sicherzustellen, dass die Eingabe als Date-Eigenschaft in der JCR-Eigenschaft gespeichert wird.
...
1. Sie können den vollständigen Code für den Beginn-Assistenten [hier](https://github.com/Adobe-Marketing-Cloud/aem-guides/blob/master/projects-tasks-guide/ui.apps/src/main/content/jcr_root/apps/aem-guides/projects-tasks/projects/wizards/content-approval-start/.content.xml)Ansicht werden.
...
1. Navigieren Sie in CRXDE-Lite zu der Projektvorlage, die Sie zuvor erstellt haben, `/apps/aem-guides/projects-tasks/projects/templates/authoring-project/workflows/models`.

   Fügen Sie unter dem Knoten models einen neuen Knoten namens **contentapproval** mit dem Knotentyp **nt:unstructured** hinzu. Fügen Sie dem Knoten  folgende Eigenschaften hinzu:
   ...
   >[!NOTE]
   >
   >Bei Verwendung von AEM 6.4 hat sich der Speicherort des Arbeitsablaufs geändert. Verweisen Sie die `modelId` Eigenschaft auf den Speicherort des Laufzeitarbeitsablaufmodells unter `/var/workflow/models/aem-guides/content-approval-workflow`
   >
   >
   >Weitere Informationen zur Änderung des Standorts des Workflows finden Sie [hier.](https://helpx.adobe.com/experience-manager/6-5/sites/developing/using/workflows-best-practices.html#LocationsWorkflowModels)
