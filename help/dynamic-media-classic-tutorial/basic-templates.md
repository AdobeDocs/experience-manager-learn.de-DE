...
In Begriffen von Dynamic Media Classic ist eine Vorlage ein Dokument, das nach der Veröffentlichung der Vorlage dynamisch über die URL geändert werden kann. Dynamic Media Classic-Angebote Grundlegende Vorlagen, bildbasierte Vorlagen, die vom Image-Server aufgerufen werden und aus Bildern und gerendertem Text bestehen.
...
Die Definition einer einfachen Vorlage (oder kurz &quot;Vorlage&quot;) ist ein URL-adressierbares Bild mit Ebenen. Das Endergebnis ist ein Bild, das jedoch durch die URL geändert werden kann. Sie kann aus Fotos, Text oder Grafiken bestehen — jede Kombination von P-TIFF-Assets in Dynamic Media Classic.
...
6. Veröffentlichen Sie die URL, kopieren Sie sie aus der Vorschau &quot;Dynamisch Media Classic&quot;und testen Sie sie in einem Browser.

## Vorbereiten und Hochladen von Vorlagenelementen zu Dynamic Media Classic
...
2. Einige Ebenentypen und Ebeneneffekte werden von Dynamic Media Classic oder dem Image-Server nicht unterstützt und müssen vor dem Hochladen gerastert werden. Andernfalls können die Effekte ignoriert oder die Ebenen verworfen werden. Das Rastern einer Ebene bedeutet, dass eine Ebene konvertiert werden muss, wenn sie bearbeitet werden kann und nicht bearbeitet werden kann. Um Ebeneneffekte oder Textebenen zu rastern, erstellen Sie eine leere Ebene, wählen Sie &quot; **Ebenen&quot;> &quot;Ebenen** zusammenführen&quot;oder &quot;STRG + E/CMD + E&quot;aus und führen Sie sie zusammen.

   - Dynamische Medien Classic können Ebenen nicht gruppieren oder verknüpfen. Alle Ebenen in einer Gruppe oder einem verknüpften Satz werden in separate Ebenen konvertiert, die nicht mehr gruppiert/verknüpft sind.
...
Sie können Ihre Schriftarten auch hochladen und veröffentlichen, wenn Sie dynamischen Text generieren müssen. Die einzige in Dynamic Media Classic enthaltene Schrift ist Arial.
...
Dynamic Media Classic unterstützt Standardschriftarten wie OpenType (OTF), TrueType (TTF) und Type 1 Postscript. Schreibgeschützte Kofferschriftarten, Typkollektionsdateien, Windows-Systemschriftarten und proprietäre Maschinenschriftarten (wie Schriftarten, die von Gravierungs- oder Stickereimaschinen verwendet werden) werden nicht unterstützt — Sie müssen sie in eines der Standardschriftformate konvertieren oder eine ähnliche Schriftart ersetzen, die Sie in Dynamic Media Classic und auf dem Image-Server verwenden können.

Nachdem Schriftarten wie jedes andere Asset in Dynamic Media Classic hochgeladen wurden, müssen sie auch auf dem Image-Server veröffentlicht werden. Ein sehr häufiger Vorlagenfehler besteht darin, die Veröffentlichung der Schriftarten zu vergessen, was zu einem Bildfehler führt — der Image-Server ersetzt keine andere Schrift an ihrer Stelle. Wenn Sie außerdem beim Hochladen die Option &quot;Text **extrahieren** &quot;verwenden möchten, müssen Sie die Schriftartdateien hochladen, bevor Sie die PSD-Datei hochladen, die diese Schriftarten verwendet. Mit der Funktion Text **extrahieren** wird versucht, den Text als bearbeitbare Textebene neu zu erstellen und ihn in einer Vorlage von Dynamic Media Classic zu platzieren. Dies wird im nächsten Thema, PSD-Optionen, erläutert.

Beachten Sie, dass Schriftarten mehrere interne Namen haben, die sich oft von ihrem externen Dateinamen unterscheiden. Sie können alle verschiedenen Namen auf der Seite &quot;Details&quot;für dieses Asset in Dynamic Media Classic anzeigen. Im Folgenden finden Sie die Namen der Schrift Adobe Caslon Pro Semibold, die auf der Registerkarte &quot;Metadaten&quot;in &quot;Dynamic Media Classic&quot;aufgeführt wird:
...
5. Ziehen Sie nach Bedarf weitere Bildebenen hinzu. hinzufügen Ebeneneffekte, wenn Sie möchten. Siehe das Thema _Hinzufügen von Ebeneneffekten_.
...
Erfahren Sie mehr[über das Hinzufügen von Ebeneneffekten](https://docs.adobe.com/content/help/en/dynamic-media-classic/using/template-basics/creating-template.html#using-shadow-and-glow-effects-on-layers).
...
In Begriffen von Dynamic Media Classic ist ein Parameter eine Variable, die mit einer Vorlageneigenschaft verknüpft werden kann, damit sie über eine URL manipuliert werden kann. Wenn Sie einen Parameter zu einer Ebene hinzufügen, legt Dynamic Media Classic diese Eigenschaft in der URL offen, indem dem Namen Ihres Parameters ein Dollarzeichen ($) vorangestellt wird. — — Wenn Sie beispielsweise einen Parameter namens &quot;Größe&quot;erstellen, um die Größe einer Ebene zu ändern, benennt Dynamic Media Classic Ihren Parameter $size um.

Wenn Sie keinen Parameter für eine Eigenschaft hinzufügen, bleibt diese Eigenschaft in der Datenbank für dynamische Medien Classic verborgen und wird nicht in der URL angezeigt.
...
1. Wählen Sie die Option &quot; **An** &quot;neben dem Namen jeder Eigenschaft, die Sie in einen Parameter umwandeln möchten. Daraufhin wird ein standardmäßiger Parametername angezeigt. Sie können nur Parameter zu Eigenschaften hinzufügen, die vom Standardstatus abweichen.

   - Wenn Sie z. B. eine Ebene hinzufügen und sie an ihrer standardmäßigen xygen Position 0,0 belassen, stellt Dynamic Media Classic keine **Position** -Eigenschaft bereit. Um die Ebene zu beheben, verschieben Sie sie um mindestens ein Pixel. Jetzt stellt Dynamic Media Classic die **Position** als Eigenschaft zur Verfügung, die parametrisiert werden kann.
   - Um der Eigenschaft &quot;Einblenden/Ausblenden&quot;einen Parameter hinzuzufügen (wodurch die Ebene aktiviert bzw. deaktiviert wird), klicken Sie auf das Symbol &quot;Ebene **einblenden** &quot;bzw. &quot;Ebene **** ausblenden&quot;, um die Ebene zu deaktivieren (wenn gewünscht, können Sie sie später wieder aktivieren). Dynamic Media Classic stellt jetzt eine Eigenschaft **Ausblenden** bereit, die parametrisiert werden kann.
...
1. Klicken Sie auf **Schließen** , um den Anzeigebereich &quot;Parameter&quot;zu verlassen.
...
Weitere Informationen zu [Vorlagenparametern](https://docs.adobe.com/content/help/en/dynamic-media-classic/using/template-basics/creating-template-parameters.html).
...
Um Text zu erstellen, müssen Sie einige Schriftarten hochladen — Andernfalls wird &quot;Dynamic Media Classic&quot;standardmäßig auf &quot;Arial&quot;gesetzt. Die Schriftarten müssen auch auf dem Image-Server veröffentlicht werden, oder es wird ein Fehler ausgegeben, sobald versucht wird, Text zu rendern, der diese Schriftart verwendet.
...
Die gute Nachricht ist, dass Dynamic Media Classic das RTF für Sie generiert. Wenn Sie Text in eine Vorlage eingeben und Formatierungen hinzufügen, schreibt Dynamic Media Classic den RTF-Code leise automatisch in die Vorlage. Der Grund, warum wir es erwähnen, ist, weil Sie Parameter direkt zu der RTF selbst hinzufügen, daher ist es wichtig, dass Sie ein wenig damit vertraut sind.
...
2. Text extrahieren (beim Hochladen). Die andere Methode besteht darin, die Textebene in Photoshop zu erstellen und sie als normale Textebene im PSD zu speichern (anstatt sie als Bildebene zu rastern). Laden Sie die Datei dann in Dynamic Media Classic hoch und verwenden Sie die Option **Text** extrahieren. Dynamic Media Classic konvertiert jede Photoshop-Textebene mithilfe von RTF-Befehlen in eine Image Serving-Textebene. Wenn Sie diese Methode verwenden, stellen Sie sicher, dass Sie Ihre Schriftarten zuerst in Dynamic Media Classic hochladen. Andernfalls ersetzt Dynamic Media Classic beim Hochladen eine Standardschriftart und es ist nicht einfach, die richtige Schriftart erneut zu ersetzen.
...
Die meisten Arbeiten werden auf der Registerkarte &quot; **Vorschau** &quot;ausgeführt, auf der Sie Text eingeben und so anzeigen können, wie er in der Vorlage aussehen wird. Es gibt auch eine Registerkarte **Quelle** , die verwendet wird, um die RTF manuell zu bearbeiten, falls erforderlich.

Der allgemeine Arbeitsablauf besteht darin, auf der Registerkarte &quot; **Vorschau** &quot;Text einzugeben.

Wählen Sie dann den Text aus und wählen Sie mit den Steuerelementen oben Formatierungen wie Schriftfarbe, Schriftgröße oder Ausrichtung aus. Nachdem der Text wie gewünscht formatiert wurde, klicken Sie auf &quot; **Übernehmen** &quot;, um die Aktualisierung in der Vorschau des Arbeitsbereichs anzuzeigen. Schließen Sie dann den Texteditor, um zum Hauptfenster &quot;Vorlagen aus Grundelementen&quot;zurückzukehren.
...
1. Klicken Sie auf die Schaltfläche **Text** -Tool oben auf der Seite Build.
...
3. Geben Sie den Beispieltext ein, der beim ersten Laden der Vorlage angezeigt werden soll. Wenn Sie z. B. ein Textfeld für ein personalisiertes E-Mail-Bild erstellen, lautet Ihr Text möglicherweise &quot;Hi Name&quot;. Jetzt ist die Zeit zu sparen!&quot; Später würden Sie einen Textparameter hinzufügen, um den Namen durch einen Wert zu ersetzen, den Sie in der URL senden. Der Text wird erst dann in der Vorlage unter dem Fenster angezeigt, wenn Sie auf **Übernehmen** klicken.
4. Um den Text zu formatieren, wählen Sie ihn durch Ziehen mit der Maus aus und wählen Sie ein Formatierungssteuerelement in der Benutzeroberfläche aus.
   ...
   - Um eine andere Schrift auszuwählen, müssen Sie den Text auswählen und das Menü &quot;Schrift&quot;öffnen. Der Editor zeigt eine Liste aller Schriftarten, die in Dynamic Media Classic hochgeladen wurden. Wenn eine Schriftart auch auf Ihrem Computer installiert ist, wird sie schwarz angezeigt. Wenn es nicht auf Ihrem Computer installiert ist, wird es rot angezeigt. Sie wird jedoch weiterhin im Fenster &quot;Vorschau&quot;angezeigt, wenn Sie auf &quot; **Übernehmen&quot;klicken**. Sie müssen Schriftarten nur in Dynamic Media Classic hochladen, damit sie für jeden verfügbar sind, der mit Dynamic Media Classic arbeitet. Nach der Veröffentlichung verwendet der Image-Server diese Schriftarten, um den Text zu generieren — Ihre Benutzer müssen keine Schriftarten installieren, um den von Ihnen erstellten Text anzuzeigen, da er Teil eines Bildes ist.
   - Im Gegensatz zu Photoshop und Illustrator kann der Image-Server den Text im Textfeld vertikal ausrichten. Die Standardeinstellung ist die Ausrichtung oben. Um dies zu ändern, wählen Sie den Text aus und wählen Sie im Menü &quot; **Vertikale Ausrichtung&quot;die Option &quot;** Mittel **&quot;oder &quot;** Unten **&quot;** .
...
Sie haben genügend Informationen, um Ihre Parameter zu erstellen, es gibt jedoch einen vollständigen Verweis auf alle RTF-Befehle in der Image Serving-Dokumentation. Rufen Sie die [Image Serving-Dokumentation](https://docs.adobe.com/content/help/en/dynamic-media-developer-resources/image-serving-api/image-serving-api/http-protocol-reference/text-formatting/c-text-formatting.html#concept-0d3136db7f6f49668274541cd4b6364c)auf.
...
1. Klicken Sie auf **Schließen** , um den Anzeigebereich &quot;Parameter&quot;zu verlassen. Klicken Sie dann auf **Speichern** , um die Vorlage zu speichern. Wenn Sie die Bearbeitung abgeschlossen haben, drücken Sie die **Taste Schließen** , um die Seite &quot;Vorlagen aus Grundelementen&quot;zu verlassen.
...
Sie müssen eine Veröffentlichung ausführen, um zu sehen, wie die Änderungen live auf dem Image-Server veröffentlicht werden. Denken Sie daran, was veröffentlicht werden muss: die verschiedenen Bildelementebenen, die Schriftarten für dynamischen Text und die Vorlage selbst. Ähnlich wie andere Rich-Media-Classic-Assets wie Bildsätze und Rotationssets ist eine einfache Vorlage eine künstliche Konstruktion — Es ist ein Zeilenelement in der Datenbank, das mithilfe einer Reihe von Image Serving-Befehlen auf die Bilder und Schriftarten verweist. Wenn Sie also die Vorlage veröffentlichen, aktualisieren Sie lediglich Daten auf dem Image-Server.

Erfahren Sie mehr über das [Veröffentlichen Ihrer Vorlage](https://docs.adobe.com/content/help/en/dynamic-media-classic/using/template-basics/publishing-templates.html).
...
![image](assets/basic-templates/sandals-without-with-parameters.png)_Beispiele einer Vorlage ohne Parameter (oben) und mit Parametern (unten)._
